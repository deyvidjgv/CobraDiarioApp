...
const MARCA = 'FF111111'; // brand principal oscuro del sistema
const MARCA_CLARA = 'FF4B5563'; // gris oscuro corporativo
const HEADER_FILL = 'FF1F2937';
const ZEBRA = 'FFF5F5F4';
const GRIS = 'FF6B7280';
...
function banner(ws, { negocio, titulo, fecha, columnas }) {
  ...
  c1.value = negocio || 'CrediDev';
  c1.font = { bold: true, size: 10, color: { argb: 'FFD1D5DB' } };
...
export async function exportarCarteraExcel(datosCartera, nombreNegocio = '') {
  ...
  const ws = wb.addWorksheet('Resumen', {
    properties: { tabColor: { argb: MARCA } },
  });
  aplicarAnchos(ws, [38, 20]);
  banner(ws, {
    negocio: nombreNegocio,
    titulo: 'REPORTE DE CARTERA GLOBAL',
    fecha: `Fecha de corte: ${datosCartera.fecha}`,
    columnas: 2,
  });
  ...
  // ─── Hoja: Créditos ───
  const wsC = wb.addWorksheet('Créditos', {
    properties: { tabColor: { argb: MARCA_CLARA } },
  });
  aplicarAnchos(wsC, [30, 10, 13, 15, 15, 15, 17, 12]);
  banner(wsC, {
    negocio: nombreNegocio,
    titulo: 'DETALLE DE CRÉDITOS',
    fecha: `Cartera al ${datosCartera.fecha}`,
    columnas: 8,
  });
  ...
  filas.forEach((d, idx) => {
    ...
    if (d.estado === 'Activo')
      est.font = { bold: true, size: 10, color: { argb: 'FF1D4ED8' } };
    else if (d.estado === 'Completado')
      est.font = { bold: true, size: 10, color: { argb: VERDE_TXT } };
  ...
  if (filas.length > 0) {
    wsC.autoFilter = {
      from: { row: 5, column: 1 },
      to: { row: 5 + filas.length, column: 8 },
    };
  }

  // ─── Hoja: Mora ───
  const wsM = wb.addWorksheet('Mora', {
    properties: { tabColor: { argb: 'FFDC2626' } },
  });
  aplicarAnchos(wsM, [30, 12, 18, 16, 18]);
  banner(wsM, {
    negocio: nombreNegocio,
    titulo: 'MORA Y ANTIGÜEDAD',
    fecha: `Corte ${datosCartera.fecha}`,
    columnas: 5,
  });
  ...
  await descargar(
    wb,
    nombreArchivo('Cartera', nombreNegocio, datosCartera.fecha),
  );
}
...
export async function exportarCierreExcel(cierre, nombreNegocio = '') {
  ...
  const efTxt =
    ef == null ? null : ef >= 80 ? VERDE_TXT : ef >= 50 ? AMBAR_TXT : ROJO_TXT;
  const efeKpis = [
  ...
  const arqKpis = [
    [
      ...
      arq.saldoEsperado ?? cierre.saldoNeto ?? 0,
      MONEDA,
      null,
    ],
    [
  ...
  // ─── Hoja: Cobros ───
  const wsCo = wb.addWorksheet('Cobros', {
    properties: { tabColor: { argb: VERDE_TXT } },
  });
  ...
  filaTotales(
    wsCo,
    6 + cobros.length + 1,
    ['', `TOTAL — ${cobros.length} cobros`, totalCobros, '', '', ''],
    [2],
  );
  ...
  if (cobros.length > 0) {
    wsCo.autoFilter = {
      from: { row: 5, column: 1 },
      to: { row: 5 + cobros.length, column: 6 },
    };
  }

  // ─── Hoja: Caja y Mora ───
  const wsCm = wb.addWorksheet('Caja y Mora', {
    properties: { tabColor: { argb: 'FFB45309' } },
  });
  aplicarAnchos(wsCm, [10, 18, 32, 14, 20]);
  banner(wsCm, {
    negocio: nombreNegocio,
    titulo: 'MOVIMIENTOS DE CAJA Y MORA',
    fecha: `Fecha: ${cierre.fecha}`,
    columnas: 5,
  });
