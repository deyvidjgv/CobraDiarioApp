...
const inputCls =
  "w-full rounded-xl bg-gray-50 border border-[#E5E5EA] pl-10 pr-4 py-3 text-sm text-gray-800 placeholder-gray-300 focus:bg-white focus:outline-none focus:border-primary-light focus:ring-2 focus:ring-primary-light/20 transition";
...
export default function Registro() {
  ...
  if (yaHabiaSesion.current) return <Navigate to="/" replace />;
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (index) {
        try {
          await saveSettings(index.organizationId, { nombreNegocio: negocio || "" });
        } catch {
    ...
    <div className="min-h-screen flex items-center justify-center bg-primary-bg p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-md sm:max-w-4xl bg-white rounded-3xl shadow-xl overflow-hidden grid grid-cols-1 sm:grid-cols-2 border border-[#E5E5EA]">
        ...
        <div className="bg-primary p-8 sm:p-10 flex flex-col justify-center items-center text-center relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center gap-4">
            <div className="w-20 h-20 rounded-2xl bg-primary-light/30 flex items-center justify-center shadow-inner">
              <IconBuildingStore size={42} stroke={1.5} className="text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-white tracking-tight">CobraDiario</h1>
              <p className="text-white/70 text-sm mt-1">
                Crea la cuenta de tu negocio y empieza a cobrar
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-10 flex flex-col justify-center bg-white">
          <h2 className="text-2xl font-bold text-primary mb-1">Crear organización</h2>
          <p className="text-sm text-gray-400 mb-6">
            Serás el <strong>Administrador</strong> de tu negocio: tus datos quedan aislados y
            tú creas las cuentas de tus cobradores.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1.5">Tu nombre</label>
              <div className="relative">
                <IconUser
                  size={18}
                  stroke={1.5}
